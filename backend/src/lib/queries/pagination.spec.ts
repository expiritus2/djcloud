...
describe('paginationQuery', () => {
  describe('simplePagination', () => {
    ...
    it('should include search query', () => {
      ...
      const queryBuilder = simplePaginateQuery(
        mockQueryBuilder as SelectQueryBuilder<any>,
        query,
      );
    ...
    it('should include skip number if page is in query', () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        expect(mockQueryBuilder.skip).toHaveBeenNthCalledWith(
          i + 1,
          skipNumber,
        );
        expect(queryBuilder).toEqual(mockQueryBuilder);
    ...
    it('should include all queries', () => {
      ...
      expect(mockQueryBuilder.skip).toBeCalledWith(
        toNumber(query.page) * toNumber(query.limit),
      );
      expect(mockQueryBuilder.orderBy).toBeCalledWith(
        `"${query.field}"`,
        query.sort,
