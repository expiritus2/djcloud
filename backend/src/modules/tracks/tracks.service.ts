...
export class TracksService {
  ...
  async getAll(
  ...
  ): Promise<{ data: TrackEntity[]; count: number }> {
    ...
    const paginateQueryBuilder = simplePaginateQuery<TrackEntity>(
      queryBuilder,
      query,
  ...
  async remove(id: string | number): Promise<TrackEntity> {
    try {
      ...
      throw new InternalServerErrorException(
        `Can not delete track with id: ${id}`,
      );
    }
  }
  ...
  async removeFile(id: number): Promise<FileEntity> {
    try {
      ...
      throw new InternalServerErrorException(
        `Can not delete file with id: ${id}`,
      );
    }
  }
}
