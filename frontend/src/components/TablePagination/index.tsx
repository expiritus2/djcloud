...
const TablePagination: FC<ComponentProps> = (props) => {
  ...
  const pages = useMemo(() => {
    ...
    const prevPages =
      current > 0
        ? new Array(
            current + 1 === countPages || current + 2 === countPages
              ? pagesOffset * 2 - 1
              : pagesOffset
          )
            .fill(0)
  ...
  return (
    <div className={classNames(styles.tablePagination, className)}>
      <div className={classNames(styles.controls)}>
        ...
        <div
          onClick={onClickPrev}
          className={classNames(styles.page, { [styles.disabled]: isFirstPage })}
        >
          Prev
        </div>
      ...
      <div className={styles.pages}>
        {pages.prev.map((page) => (
          <div
            onClick={(e) => onClickPageHandler(e, page)}
            key={page}
      ...
      {pages.next[pages.next.length - 1] !== pages.countPages &&
      current + 1 !== pages.countPages ? (
        <>
      ...
      <div className={styles.controls}>
        <div
          onClick={onClickNext}
          className={classNames(styles.page, { [styles.disabled]: isLastPage })}
        >
          Next
        </div>
        <div
