...
export default (refs: any, handler: Function, eventNames = ['pointerup', 'keyup', 'touchend']) => {
  ...
  if (typeof handler !== 'function')
    throw new Error('The `handler` is not a function in useOutsideClick');

  const handlerWithCheck = useCallback(
    (event: any) => {
      if (Array.isArray(refs)) {
        const isNotCurrentRef = refs.every(
  ...
  useEffect(() => {
    ...
    return () =>
      eventNames.forEach((eventName) => document.removeEventListener(eventName, handlerWithCheck));
