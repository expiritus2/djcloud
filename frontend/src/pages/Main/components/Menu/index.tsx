...
const MainMenu: FC<ComponentProps> = (props) => {
  ...
  const mapTrackGenresByCategory = useCallback(
    (
      [categoryId, genres]: [string, TrackGenre[]],
      groupedGenres: { [key: string]: TrackGenre[] }
    ) => {
      return genres
        .map((genre: TrackGenre, index: number) => {
          const item = convertMenuItem(genre, index, categoryId);
          const cat = (navCategories.data?.data || []).find(
  ...
  const menuItems = useMemo(() => {
    ...
    if (match?.params.categoryId) {
      const categoryId =
        match?.params.categoryId || navCategories.data?.data?.[0]?.id.toString() || '';
      if (categoryId) {
        list = ((tracksGenres.data as GroupedTrackGenres)?.[+categoryId] || []).map(
          (genre, index) => convertMenuItem(genre, index, categoryId)
        );
      }
    }
  ...
  return (
    <Menu
      ...
      switcher={
        location.pathname !== routes.allTracks && location.pathname !== routes.index ? (
          <MenuSwitcher />
        ) : null
      }
    />
