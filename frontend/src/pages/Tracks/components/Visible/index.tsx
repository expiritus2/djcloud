...
const Visible: FC<ComponentProps> = (props) => {
    ...
    const onVisibleChange = useCallback(
        (e: any) => {
            setPending(true);
            modifyTrack.updateVisible({ id: track.id, visible: e.target.value }, {}, (err: AxiosError) => {
                if (!err) {
                    setPending(false);
    ...
    return (
        <div className={classNames(styles.visible, className)}>
            <Checkbox pending={pending} name="visible" checked={track.visible} onChange={onVisibleChange} />
        </div>
    );
};
